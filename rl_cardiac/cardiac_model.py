...
class CardiacModel_Env(Env):
    def __init__(self, tcn_model, rat_type, noise_level=0.0):
        ...
        low = np.array(
            [self.min_HR, self.min_MAP, self.min_HR, self.min_MAP], dtype=np.float32
        )
        high = np.array(
            [self.max_HR, self.max_MAP, self.max_HR, self.max_MAP], dtype=np.float32
        )
        self.observation_space = spaces.Box(low=low, high=high, dtype=np.float32)
        self.sp_list = [
            np.array([0.074, -0.37]),
            np.array([-0.355, 0.108]),
            np.array([0.064, -0.16]),
        ]
    ...
    def step(self, u):
        ...
        self.state = self.tcn_model.predict(in_TCN)[0][0] + (
            np.random.normal(0, self.noise_level * 0.5, np.shape(self.state))
        )
        ...
        if self.consider_sp:
            hist = (
                (np.array(self.listhistory + list(self.setpoints.flatten())))
                .reshape(1, -1)
                .flatten()
            )
